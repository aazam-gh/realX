...
function parseInput(data: Input) {
  ...
  if (
    ...
    StudentType !== "all" &&
    StudentType !== "in_store" &&
    StudentType !== "online"
  ) {
    throw new HttpsError("invalid-argument", "Unsupported StudentType");
  }
  ...
  return {
    ...
    StudentType: typeof StudentType === "string" ? StudentType : "all",
...
export async function listAdminBigQueryStudentsHandler(
  ...
  const {pageSize, search, StudentType, xcard, sort, cursor} =
  ...
  if (StudentType !== "all") {
    where.push("Student_type = @StudentType");
    params.StudentType = StudentType;
    types.StudentType = "STRING";
  }
  ...
  try {
    ...
    return {
      Students: pageRows.map((row) => ({
        ...
        StudentType: row.Student_type || "in_store",
